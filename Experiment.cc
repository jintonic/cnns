...
Double_t Experiment::XSxN2(Double_t *x, Double_t *parameter)
{
   Double_t Ev = x[0]*MeV; // neutrino energy
   Double_t Er = parameter[0]*keV; // nuclear recoil energy
   UShort_t type = static_cast<UShort_t>(parameter[1]); // type of neutrino
   ...
   if (type<1 || type>6)
      return dXS*(fSource->N2(1,time,Ev)/second/MeV
            + fSource->N2(2,time,Ev)/second/MeV 
            + 4*fSource->N2(3,time,Ev))/second/MeV;

   return dXS * fSource->N2(type,time,Ev)/second/MeV;
}
...
Double_t Experiment::N2(UShort_t type, Double_t time, Double_t Enr)
{
   if (!fMaterial) {
      Warning("Nevt", "Please set targe material!");
      return 0;
...
TF1* Experiment::FNevt(UShort_t type, Double_t maxEr, Double_t maxEv)
{
   if (fNevt[type]) {
      Double_t min, max;
   ...
   fNevt[type] = new TF1(Form("fNevt_%s_%s_%f_%d",
         ...
         this, &Experiment::FuncN, 0, maxEr/keV,2);
   fNevt[type]->SetParameter(0,maxEv/MeV);
...
TF1* Experiment::FXSxNe(UShort_t type, Double_t nEr,
      Double_t minEv, Double_t maxEv)
{
   ...
   fXSxNe[type] = new TF1(Form("fXSxNe%s%s%f%d",
         ...
         &Experiment::XSxNe, minEv/MeV, maxEv/MeV,2);
   fXSxNe[type]->SetParameter(0,nEr/keV);
...
TF1* Experiment::FXSxN2(UShort_t type, Double_t time, 
      Double_t Enr, Double_t minEv, Double_t maxEv)
{
   ...
   fXSxN2[type] = new TF1(Form("fXSxN2%s%s%f%d",
            fSource->GetName(), fMaterial->GetName(), fMass, type), this, 
         &Experiment::XSxN2, minEv/MeV, maxEv/MeV,3);
   fXSxN2[type]->SetParameter(0,Enr/keV);
   ...
   if (type==0) {
      fXSxN2[type]->SetTitle(Form(
               ...
               fSource->GetName(), fMaterial->GetTitle(), Enr/keV, time/second));
...
TH1D* Experiment::HNevt(UShort_t type, Double_t maxEr, Double_t maxEv)
{
...
TH1D* Experiment::HXSxNe(UShort_t type, Double_t nEr,
      Double_t minEv, Double_t maxEv)
{
...
TH2D* Experiment::HN2(UShort_t type)
{
   if (fHN2[type]) return fHN2[type];

   Int_t nbinst=fSource->NbinsT();
   const Double_t *tbins = fSource->BinEdgesT()->GetArray();
   ...
   for (Int_t ix=1; ix<=fHN2[type]->GetNbinsX(); ix++) {
      for (Int_t iy=1; iy<=fHN2[type]->GetNbinsY(); iy++) {
         Double_t time = fHN2[type]->GetXaxis()->GetBinCenter(ix);
         Double_t enur = fHN2[type]->GetYaxis()->GetBinCenter(iy);
         Double_t nevt = N2(type,time*second,enur*keV);
...
TH1D* Experiment::HNt(UShort_t type)
{
